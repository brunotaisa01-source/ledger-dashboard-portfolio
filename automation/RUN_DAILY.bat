...
title Dashboard Ledger Operations Pack - RECURRING Refresh
color 0A
...
set "LOGFILE=%PROJECT%\runtime\logs\daily_%RUN_ID%.log"

echo =====================================================
echo   Dashboard Ledger Local Fixture Store Pack
echo   RUN REFRESH - Loaders + Dashboard + Validation
...
%PY_CMD% -m scripts.tools.preflight_pack --mode daily >> "%LOGFILE%" 2>&1
if errorlevel 1 (
...
%PY_CMD% "%RUN_LOCK%" acquire --mode daily >> "%LOGFILE%" 2>&1
...
REM ---- Pipeline (no monthly / no upstream reports) ------------
...
call "%~dp0..\lib\run_dashboard_step.bat" "%LOGFILE%" "Dashboard local HTML" daily --reuse-trend-cube
if errorlevel 1 (
  set /a FAILS+=1
  goto fail_required
)
...
if not "%FAILS%"=="0" (
  echo [FAIL] Daily refresh finished with required step failures.
  if not "%NOPAUSE%"=="1" pause
  exit /b 1
)

echo [OK] Daily refresh completed. Open dashboard\dashboard.html to view.
if not "%NOPAUSE%"=="1" pause
exit /b 0
...
%PY_CMD% -m scripts.dashboard.Rol_Query --force-html --local-only --reuse-trend-cube >> "%LOGFILE%" 2>&1
...
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0Verify-DashboardArtifacts.ps1" -OutputDir "%DASHBOARD_OUTPUT_DIR%" -CutoffEpoch "%DASH_START_EPOCH%" -Mode "daily" >> "%LOGFILE%" 2>&1
