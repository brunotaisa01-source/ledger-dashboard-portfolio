...
echo   RUN REBUILD ALL - Full Dashboard Cache Rebuild + Validation
...
if errorlevel 1 set /a WARNS+=1
call "%~dp0..\lib\run_dashboard_step.bat" "%LOGFILE%" "Dashboard local HTML" rebuild-all
if errorlevel 1 (
  set /a FAILS+=1
  goto fail_required
)
...
echo [OK] Rebuild-all cycle completed. Open dashboard\dashboard.html to view.
if not "%NOPAUSE%"=="1" pause
exit /b 0
...
%PY_CMD% -m scripts.dashboard.Rol_Query --force-html --local-only >> "%LOGFILE%" 2>&1
...
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0Verify-DashboardArtifacts.ps1" -OutputDir "%DASHBOARD_OUTPUT_DIR%" -CutoffEpoch "%DASH_START_EPOCH%" -Mode "rebuild-all" >> "%LOGFILE%" 2>&1
